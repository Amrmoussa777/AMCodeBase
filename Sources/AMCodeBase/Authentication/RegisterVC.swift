...
class RegisterVC: UIViewController {
        ...
        if counter < 3 {
            ...
            case 1:
                if userNameFeild.text!.isValidUserName {
                    ...
                    view.showAlertView(avatarImage: AlertImages.lockImage!, Message: "Please add a valid User name ...", buttonLabel: "OK", buttonImage: Images.nextButton!,actionButtonColor: .blue)
    ...
    private func checkEmailalreadyRegistered(email:String?){
        ...
        AMNetworkManager.Shared.checkEmailExist(email: validEmail) { [weak self] isExist in
            guard let self = self else{return}
           
            switch(isExist){
            case true:
            // enail  exists in DB
                self.view.showAlertView(avatarImage: AlertImages.lockImage!, Message: "Email already exists in our data base .", buttonLabel: "OK", buttonImage: Images.nextButton!,actionButtonColor: .blue)
